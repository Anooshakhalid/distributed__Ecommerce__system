...
def orders():
    try:
        if request.method == "GET":
            res = requests.get(f"{ORDER_SERVICE}/orders")
            return jsonify(res.json())

        res = requests.post(f"{ORDER_SERVICE}/order", json=request.json)
        return jsonify(res.json())
...
def payment():
    try:
        res = requests.post(f"{PAYMENT_SERVICE}/payment", json=request.json)
        return jsonify(res.json())
    except Exception as e:
        return jsonify({"error": str(e)}), 500
